...
抽离到 microprofile.config 到单独的模块「configuration」中，同样是依赖 SPI 机制加载 `ServletContainerInitializer` 的实现类，在 `onStartup` 初始化 `ConfigProviderResolver`。
